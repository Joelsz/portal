...
IO.controller('PortalHomeController', function($scope){
	...
	var day_data = [
		...
		{"elapsed": "2013 - 12", "value": 83, b: 29},
		{"elapsed": "2014 - 01", "value": 90, b: 23},
		{"elapsed": "2014 - 02", "value": 53, b:10},
		{"elapsed": "2014 - 03", "value": 80, b:22},
		{"elapsed": "2014 - 04", "value": 65, b:7},
		{"elapsed": "2014 - 05", "value": 70, b:6},
		{"elapsed": "2014 - 06", "value": 85, b:17},
		{"elapsed": "2014 - 07", "value": 63, b:15},
		{"elapsed": "2014 - 08", "value": 45, b:17},
		{"elapsed": "2014 - 09", "value": 87, b:7},
...
IO.factory('Banner', function($resource) {
    return $resource('/api?O=home&Q=json:action', {}, {
        save: {
            method: 'POST',
...
IO.factory('Alerts', function($resource) {
	return $resource('/api?O=home&A=GetAlerts&Q=json&filter=false');
});

IO.factory('Groups', function($resource) {
	return $resource('/api?O=home&A=GetGroups&Q=json');
});

IO.factory('SQA', function($resource) {
    return $resource('/api?O=home&Q=json&A=DoSQACall', {}, {
        save: {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
        },
